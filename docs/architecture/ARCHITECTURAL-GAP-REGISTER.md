...
| H-07 | Necesidades no priorizadas (`UnaddressedNeed[]`) integradas en Plan de Acción | Implementación pendiente (parcial) | `LocalHealthPlanDocument.ts`; `CONTRACT-LOCAL-HEALTH-PLAN-COMPILER.md`; `PlanPreparationDraft.unaddressedNeeds`; `PlanPreparationPanel`; `PlanDocument.unaddressedNeeds`; `validatePlanDocumentForLocalHealthPlan` | Soporte de dominio, captura UI y transporte documental implementados (2026-09-24); consumo por `LocalHealthPlanCompiler` pendiente | Gates del PLS | Media |

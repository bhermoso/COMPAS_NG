...
import {accessError,createTerritorialAccount,listManagedAccounts,saveManagedAccess,type ManagedAccount,type NewAccount} from '../../infrastructure/relas/AdminAccounts';
...
export default function AdministrationPanel({client,onOpenApp,onOpenScope}:{client:RelasClient;onOpenApp:()=>void;onOpenScope:(scope:string)=>void}) {
 ...
 useEffect(()=>{let live=true;void listManagedAccounts(client).then(rows=>{if(live)setAccounts(rows);}).catch(e=>{if(live)setMessage(accessError(e));});return()=>{live=false;};},[client]);
 ...
 return <>
  ...
  <section className="admin-section"><h2>Crear acceso para un plan de salud</h2><p>Indica el correo de la persona responsable y su plan. COMPAS generará la contraseña. La cuenta de coordinación podrá consultar y editar su borrador compartido; sus propuestas quedan pendientes de aprobación.</p>
   <form onSubmit={async e=>{e.preventDefault();setBusy(true);setMessage('Creando cuenta…');try{const account={...await createTerritorialAccount(client,email),scope:selectedScope,role};setCreated({...account,granted:false});await grant(account);}catch(error){setMessage(accessError(error));}finally{setBusy(false);}}}>
    <label>Plan de salud<select disabled={busy||!!created} value={selectedScope} onChange={e=>setSelectedScope(e.target.value)}>{scopes.map(s=><option key={s} value={s}>{s==='granada-zaidin'?'Granada · Zaidín':s}</option>)}</select></label>
    ...
    <button disabled={busy||!!created||!email}>Crear usuario, contraseña y acceso</button>
   </form>
  ...
  <section className="admin-section"><h2>Trabajo de COMPAS</h2><button onClick={onOpenApp}>Abrir COMPAS completo</button><div className="admin-scope"><button onClick={()=>onOpenScope(selectedScope)}>Abrir plan seleccionado</button></div></section>

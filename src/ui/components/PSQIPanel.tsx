...
import { PSQI_MODULE } from "../../domain/methodology/definitions/psqi";

interface PSQIPanelProps {
  psqiStudy?: PSQIStudy;
...
export function PSQIPanel({ psqiStudy, municipalityName }: PSQIPanelProps) {
  if (psqiStudy === undefined) return null;
  const module = PSQI_MODULE;
  const note = module.institutionalNote;
  const mun = municipalityName ?? "el municipio";
  const { aggregates: agg } = psqiStudy;
    ...
    <div className="study-report">
      <header className="study-report__header">
        ...
        <div className="study-report__header-meta">
          ...
          <span className="study-report__meta-date">Importado el {formatDate(psqiStudy.createdAt)}</span>
          <span className="study-report__meta-n">{agg.nValid} válidos · mal dormidor {agg.pctPositive.toFixed(1)} %</span>
        </div>
      </header>
      ...
      <section className="study-report__section">
        ...
        <div className="study-bar-section">
          <table className="study-bar-table">
            <tbody>
              {[
                { label: "Mala calidad del sueño (>5)", value: agg.nPositive, pct: agg.pctPositive, total: true },
                { label: "Buen dormidor (≤5)", value: agg.nScore0to5, pct: agg.nValid > 0 ? (agg.nScore0to5 / agg.nValid) * 100 : 0, total: false },
                { label: "Mal dormidor leve-moderado (6–10)", value: agg.nScore6to10, pct: agg.nValid > 0 ? (agg.nScore6to10 / agg.nValid) * 100 : 0, total: false },
                { label: "Mal dormidor grave (11–21)", value: agg.nScore11to21, pct: agg.nValid > 0 ? (agg.nScore11to21 / agg.nValid) * 100 : 0, total: false },
              ].map((row, i) => (
      ...
      <details className="study-report__collapsible">
        ...
        <div className="study-report__collapsible-body">
          <ul className="study-report__cautions-list">
            {[...module.limitations, ...psqiStudy.methodologicalCautions].map((c, i) => <li key={i}>{c}</li>)}
          </ul>
        </div>
      </details>
      ...
      <details className="study-report__collapsible">
        ...
        <div className="study-report__collapsible-body">
          <dl className="study-report__evidence-dl">
            <dt>Archivo fuente</dt><dd className="study-report__evidence-file">{psqiStudy.sourceFileName}</dd>

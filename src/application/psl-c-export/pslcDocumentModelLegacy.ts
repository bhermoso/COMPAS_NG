 ...
 *
 * Ruta B HISTÓRICA del modelo documental del PSL-C, AISLADA por GOV-SALIDA-01
 * (PR-2). Es el ÚNICO punto que reconstruye el contenido desde la instantánea
 * `provenance.diagnosticAnswersSnapshot` y desde `conclusiones.content`
 * (parseNarrativeChapters). Solo se invoca para artefactos legacy o v2
 * incompletos; los artefactos v2 completos se proyectan mecánicamente desde
 * `editorialView` + `technicalSpace` (ver `pslcCanonicalProjector`). Ningún
 * camino v2 la consume.
 *
...
export function buildPSLCDocumentModelLegacy(
